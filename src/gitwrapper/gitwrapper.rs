...
enum CliCommands {
    ...
    /// This is only if you rename the gitwrapper binary
    SetCustomSsh {
        /// Custom binary name for gitwrapper
        ssh_bin: String,
    },
}
...
fn main() {
    ...
    match args.command {
        ...
        CliCommands::Git { git_args } => {
            ...
            assert!(
                !config_yaml.priv_key.is_empty(),
                "Private key in config is empty, please use Set subcommand to configure a key."
            );
        ...
        CliCommands::View {} => {
            ...
            println!(
                "Configured settings:\nPrivate key path: {}\nWrapped ssh binary: {}",
                config_yaml.priv_key, config_yaml.ssh_bin
            );
        }
        CliCommands::Clear {} => match fs::remove_file(get_config_file()) {
...
fn call_git_command(args: &Vec<String>) -> io::Result<ExitStatus> {
    let config_yaml = get_config_yaml();
    Command::new("git")
        .env("GIT_SSH", config_yaml.ssh_bin)
        .args(args)
        .spawn()?
        .wait()
}
